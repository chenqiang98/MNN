...
static std::vector<std::string> loadKnowledge(const std::string &kbFolder) {
    std::vector<std::string> knowledgeChunks;
    DIR *dirp = opendir(kbFolder.c_str());
    if (dirp) {
        struct dirent *dp;
        while ((dp = readdir(dirp)) != NULL) {
            ...
            std::string fullPath = kbFolder + "/" + fname;
            struct stat st;
            if (stat(fullPath.c_str(), &st) == 0 && S_ISREG(st.st_mode)) {
                if (fname.size() >= 4 && fname.compare(fname.size() - 4, 4, ".txt") == 0) {
                    ...
                    while (std::getline(ifs, line)) {
                        knowledgeChunks.push_back(line);
                    }
                }
            }
        }
        std::cout << "Loaded " << knowledgeChunks.size() << " knowledge chunks from " << kbFolder << std::endl;
        closedir(dirp);
    }
    return knowledgeChunks;
}

std::string RAG(const std::string &userInput, const std::vector<std::string> &knowledgeChunks, Embedding* embedding) {
    // Prepare embedding for user input
    auto userEmbedding = embedding->txt_embedding(userInput);
    // dumpVARP(userEmbedding);

    float bestDist = 1e10f;
    float dist = 0.0f;
    std::string bestChunk, question, answer;
    for (std::string chunk : knowledgeChunks) {
        // if "answer:" in chunk, split it
        auto pos = chunk.find("\"answer\":");
        if (pos != std::string::npos) {
            question = chunk.substr(25, pos-28);
            answer = chunk.substr(pos + 9);
        }
        else {
            question = chunk;
            answer = chunk;
        }
        auto chunkEmbedding = embedding->txt_embedding(question);
        // dumpVARP(chunkEmbedding);
        float dist = Embedding::dist(userEmbedding, chunkEmbedding);
        if (dist < bestDist) {
            bestDist = dist;
            bestChunk = answer;
            // std::cout << "found best chunk: " << question << std::endl;
            // std::cout << "found best answer: " << answer << std::endl;
            // std::cout << "found best dist: " << bestDist << std::endl;
        }   
    }
    return bestChunk;
}
...
void chat(Llm* llm, const std::vector<std::string> &knowledgeChunks, Embedding* embedding) {
...
int main(int argc, const char* argv[]) {
    ...
    std::string embedding_path = "/Users/chenqiang/Desktop/2024/llm/gte_sentence-embedding_multilingual-base-MNN/embedding.mnn";
    std::unique_ptr<Embedding> embedding(Embedding::createEmbedding(embedding_path));
    ...
    // Load knowledge once:
    std::string kbFolder = "/Users/chenqiang/Desktop/2024/sem2/ARIN7102-AppliedDataMining/project/knowledge";
    auto knowledgeChunks = loadKnowledge(kbFolder);

    if (argc < 3) {
        chat(llm.get(), knowledgeChunks, embedding.get());
        return 0;
    }
